...
@Component({
  selector: 'app-admin-chairperson-details',
  standalone: true,
  imports: [CommonModule, MatButtonModule, MatIconModule, AdminSideBar],
  templateUrl: './admin-chairperson-details.html',
  styleUrls: ['./admin-chairperson-details.css']
})
export class AdminChairpersonDetails implements OnInit {
  loading = signal(true);
  ...
  approveGroup(): void {
    ...
    const userName = `${this.currentUser?.Firstname || ''} ${this.currentUser?.Lastname || ''}`.trim() || 'Chairperson';
    
    if (!confirm(`Are you sure you want to approve all milestones for group ${group.group_id}?\n\nThis will forward the thesis to the Dean for final approval.`)) {
      return;
    }
    ...
    this.http.patch(`${environment.authApiUrl}/groups/${group.group_id}/chairperson-approve-final`, {
      ...
      next: (response: any) => {
        if (response.success) {
          ...
          this.router.navigate(['/admin-chairperson-approval']);
        }
      },
      error: (error) => {
  ...
  rejectGroup(): void {
    ...
    this.http.patch(`${environment.authApiUrl}/groups/${group.group_id}/chairperson-reject`, {
      chairperson_name: userName,
      reason: reason.trim(),
