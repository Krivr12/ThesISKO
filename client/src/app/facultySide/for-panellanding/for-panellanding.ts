...
export class ForPanellanding implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get<any>('http://localhost:5050/groups', { withCredentials: true }).subscribe({
      next: (apiResponse) => {
        ...
        const rows: any[] = apiResponse?.groups || [];
        this.processGroupsData(rows);
      },
      error: (apiError) => {
        ...
        this.http.get<any>('/groups.json').subscribe({
          next: (rawData) => {
            const rows: any[] = Array.isArray(rawData) ? rawData : (rawData?.groups ?? []);
            this.processGroupsData(rows);
          },
          error: (jsonError) => {
  ...
  private processGroupsData(rows: any[]): void {
    // Minimal normalize
