...
// Configure Google OAuth Strategy (only if environment variables are available)
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
  passport.use(
    ...
    async (accessToken, refreshToken, profile, done) => {
      try {
        ...
        const photoUrl = (profile.photos && profile.photos[0] && profile.photos[0].value) ? profile.photos[0].value : null
        ...
        if (existingRows.length > 0) {
          ...
          // Optionally update name/status/googleId if changed
          const needsUpdate = (user.Firstname !== firstName) || (user.Lastname !== lastName) || (user.Status !== status) || (user.AvatarUrl !== photoUrl)
          if (needsUpdate) {
            try {
              // Get role_id for the status
              const roleId = await getRoleId(status)
              
              // Update users_info table (single table)
              await pool.execute(
                'UPDATE users_info SET role_id = ?, google_id = ?, firstname = ?, lastname = ?, avatar_url = ? WHERE LOWER(email) = ? LIMIT 1',
                [roleId, googleId, firstName, lastName, photoUrl, email]
              )
            } catch (e) {
  ...
  console.warn('⚠️  Google OAuth not configured. Set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in config.env to enable Google login.')
}
